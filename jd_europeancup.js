...
function getActCk2() {
    return new Promise(resolve => {
        $.get(taskUrl("/dingzhi/hisense/europeancup/activity/7431935", `activityId=${$.actid}&shareUuid=${$.shareuuid2}&adsource=null&initHash=/home&shareuserid4minipg=8A%2BMf3SBYE8spQtvzQ2VLE7oeVP9kq2pYSH90mYt4m3fwcJlClpxrfmVYaGKuquQkdK3rLBQpEQH9V4tdrrh0w%3D%3D&shopid=undefined&lng=107.146945&lat=33.255267&sid=cad74d1c843bd47422ae20cadf6fe5aw&un_area=8_573_6627_52446`), (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${$.name} API请求失败，请检查网路重试`)
                } else {
...
function getUid() {
    return new Promise(resolve => {
        let body = `activityId=${$.actid}&pin=${encodeURIComponent($.pin)}&pinImg=${$.pinImg}&nick=${encodeURIComponent($.nick)}&cjyxPin=&cjhyPin=&shareUuid=b7f58330cb0844b485afacbdea3c7bca&picId=`
